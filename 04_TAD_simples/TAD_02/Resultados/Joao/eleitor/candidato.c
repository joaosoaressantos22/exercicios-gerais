...
tCandidato LeCandidato(){
    tCandidato candidato; 
    scanf("\n%[^,], %[^,], %[^,], %d", candidato.nome, candidato.partido,
...
int EhMesmoCandidato(tCandidato candidato1, tCandidato candidato2){
    if (candidato1.id == candidato2.id && strcmp(candidato1.partido, candidato2.partido) == 0 && candidato1.cargo == candidato2.cargo){
        return 1;
    }
    return 0;
}
...
float CalculaPercentualVotos(tCandidato candidato, int totalVotos){
    float percentual = ((((float)candidato.votos)*100)/((float)totalVotos));
    return percentual;
}
/**
